...
export const RuleCard: React.FC<RuleCardProps> = ({ rule, onDelete, onUpdate }) => {
    const [showDeleteConfirm, setShowDeleteConfirm] = useState(false);
    const { data: tagsData } = useGetTags();


    const tagObjects = tagsData?.data?.filter((tag: TagType) =>
        ...
        <Card className="w-full bg-slate-50 border-2 border-blue-50 rounded-md shadow-md">
            <CardHeader className="flex flex-row items-center justify-between p-4 pb-2">
                ...
                <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-36">
                        <DropdownMenuItem onClick={() => onUpdate(rule)} className="cursor-pointer">
            ...
            <CardContent className="p-4 pt-0">
                <div className="flex items-center text-sm text-gray-500 mb-3">
                    <Badge variant="destructive" className="bg-vdcoffe text-gray-50 font-normal">
                        {rule.ruleType}
                    </Badge>
                </div>
