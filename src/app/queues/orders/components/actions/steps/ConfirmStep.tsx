            ...
            <div className="bg-muted/50 rounded-md p-3 space-y-2">
                ...
                <div className="text-xs text-muted-foreground">
                    {actionType?.includes("Tag") ? (
                        <>
                            ...
                            {target === "selected" ? `${orders.length} pasirinktiems` : "visiems filtruotiems"} užsakymams
                        </>
                    ) : (
                        <>
                            {getTranslatedAction(actionType)}{" "}
                            {target === "selected" ? `${orders.length} pasirinktus` : "visus filtruotus"} užsakymus
                        </>
                    )}
                </div>
                <div className="flex flex-wrap gap-1.5">
                    ...
                    <Badge variant="outline" className="text-xs px-2 py-1">
                        {target === "selected" ? `${orders.length} Pasirinkta` : "Visi filtruoti"}
                    </Badge>
                </div>
            </div>
